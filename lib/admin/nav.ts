// Admin sidebar sections. Extended as each collection editor is built.
export const adminNav: { label: string; href: string }[] = [
  { label: "Dashboard", href: "/vddbinew" },
  { label: "Pages", href: "/vddbinew/pages" },
  { label: "Services", href: "/vddbinew/services" },
  { label: "Team", href: "/vddbinew/collections/team" },
  { label: "Testimonials", href: "/vddbinew/collections/testimonials" },
  { label: "Gallery", href: "/vddbinew/collections/gallery" },
  { label: "Pricing", href: "/vddbinew/collections/pricing" },
  { label: "FAQs", href: "/vddbinew/collections/faqs" },
  { label: "Site Text", href: "/vddbinew/content" },
  { label: "Submissions", href: "/vddbinew/submissions" },
  { label: "Search Engines", href: "/vddbinew/seo" },
];
